  ...
  return (
    <div className="min-h-screen bg-gray-50 px-8 py-10">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-7 w-full max-w-md">
            ...
            <div className="space-y-4">
              {[
                { label: "Full name", type: "text", value: editName, onChange: setEditName, placeholder: "" },
                { label: "Email address", type: "email", value: editEmail, onChange: setEditEmail, placeholder: "" },
                { label: "Password", type: "password", value: editPassword, onChange: setEditPassword, placeholder: "Leave blank to keep unchanged" },
            ...
            <div className="flex gap-3 mt-7 pt-6 border-t border-gray-100">
              ...
              <button
                onClick={() =>
                  updUser(selectedUser.email, {
                    name: editName, email: editEmail, password: editPassword, role: editRole,
                  })
                }
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-7 max-w-lg shadow-sm">
        {message && (
          <div
            className={`mb-5 flex items-center gap-2.5 px-4 py-3 rounded-xl text-sm border ${
        ...
        <div className="space-y-4">
          {[
            { label: "Full name", type: "text", value: name, onChange: setName, placeholder: "Enter name" },
            { label: "Email address", type: "email", value: email, onChange: setEmail, placeholder: "Enter email" },
            { label: "Password", type: "password", value: password, onChange: setPassword, placeholder: "Enter password" },
          ].map(({ label, type, value, onChange, placeholder }) => (
            <div key={label}>
              <label className="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1.5">{label}</label>
              <input
        ...
        <div className="flex gap-3 mt-6 pt-6 border-t border-gray-100">
          ...
          <button
            ...
            className="px-5 py-2.5 border border-gray-200 text-gray-500 text-sm rounded-lg hover:bg-gray-50 transition-colors"
          >
            Clear
          </button>
        </div>
      ...
      {usersError && !usersLoading && (
        <div className="flex items-center justify-between bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-xl text-sm">
          ...
          <button onClick={getUser} className="text-red-600 underline hover:text-red-800 ml-4 font-medium">
            Retry
          </button>
        </div>
      )}
                    ...
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold shrink-0 ${
                          avatarColor[user.role] ?? "bg-gray-100 text-gray-600"
                        }`}
                      >
                  ...
                  <td className="px-5 py-4">
                    <span
                      className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        roleBadge[user.role] ?? "bg-gray-100 text-gray-600 border border-gray-200"
                      }`}
                    >
